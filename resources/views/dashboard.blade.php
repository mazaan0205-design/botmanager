...
<head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Inter:wght@400;500;600;700;800;900&amp;family=Material+Symbols+Outlined:wght,FILL@100..700,0..1&amp;display=swap"
                rel="stylesheet" />
...
<a href="{{ isset($bot) && $bot->id !== 'new' ? '/bots/preview/'.$bot->id : '#' }}" 
   ...
   Test Preview
       ...
       <button class="w-full h-10 bg-primary text-white font-label-md rounded-lg flex items-center justify-center gap-2 hover:opacity-90 active:opacity-80 transition-opacity">
    ...
    <div class="flex items-center flex-1">
        ...
        <form action="{{ route('dashboard') }}" method="GET" class="w-full max-w-[130px] sm:max-w-md">
            <div class="relative w-full">
                ...
                @if(request('search'))
                    <a href="{{ route('dashboard') }}" class="absolute right-3 top-1/2 -translate-y-1/2 text-[11px] text-slate-400 hover:text-slate-600 underline decoration-dotted">
                        Clear
                    </a>
                @endif
            </div>
...
<main class="lg:ml-[280px] pt-16 min-h-screen transition-all duration-300">
        <div class="p-4 md:p-[40px]">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-4 gap-gutter mb-xl">
                                <div
                                        class="bg-white p-lg rounded-xl border border-slate-200 flex flex-col gap-2 shadow-sm">
                                        ...
                                        <div class="flex items-center justify-between">
                                                ...
                                                <span
                                                        class="px-2 py-1 bg-green-50 text-green-700 text-xs font-bold rounded">+2
                                                        this month</span>
                                        </div>
                                </div>
                                <div
                                        ...
                                        <div class="flex items-center justify-between">
                                                ...
                                                <span
                                                        class="px-2 py-1 bg-indigo-50 text-indigo-700 text-xs font-bold rounded">Top
                                                        5%</span>
                                        </div>
                                </div>
                                <div
                                        ...
                                        <div class="flex items-center justify-between">
                                                ...
                                                <span
                                                        class="px-2 py-1 bg-indigo-50 text-indigo-700 text-xs font-bold rounded">-0.4s</span>
                                        </div>
                                </div>
                                <div
                                        class="bg-white p-lg rounded-xl border border-slate-200 flex flex-col gap-2 shadow-sm">
    ...
    @forelse($bots as $bot)
    <div class="bg-white border border-slate-200 rounded-xl overflow-hidden hover:shadow-md transition-shadow group {{ ($bot->statistics->status ?? 'Online') !== 'Online' ? 'opacity-75' : '' }}">
        <div class="p-lg">
            <div class="flex justify-between items-start mb-md">
                <div class="flex items-center gap-3">
                    <div class="relative">
                        ...
                        <span class="absolute -bottom-1 -right-1 w-4 h-4 {{ ($bot->statistics->status ?? 'Online') == 'Online' ? 'bg-green-500' : 'bg-slate-400' }} border-2 border-white rounded-full"></span>
                    </div>
                    <div>
                        <h3 class="font-h3 text-[18px] text-slate-900">{{ $bot->name }}</h3>
                        <span class="{{ ($bot->statistics->status ?? 'Online') == 'Online' ? 'text-green-600' : 'text-slate-500' }} font-label-sm flex items-center gap-1">
                            <span class="w-1.5 h-1.5 {{ ($bot->statistics->status ?? 'Online') == 'Online' ? 'bg-green-500' : 'bg-slate-400' }} rounded-full"></span>
            ...
            <div class="flex items-center justify-between pt-md border-t border-slate-50">
                <div class="flex flex-col">
                    ...
                    <span class="text-slate-700 font-label-md">{{ $bot->statistics->conversations ?? 0 }}</span>
                </div>
                <div class="flex gap-2">
                    ...
                    <div class="flex gap-2">
    <a href="/bots/preview/{{ $bot->id ?? 'new' }}" class="px-4 py-2 bg-slate-100 rounded-lg text-sm font-medium">Test</a>
<a href="/bots/edit/{{ $bot->id ?? 'new' }}" class="px-4 py-2 bg-slate-100 rounded-lg text-sm font-medium">Edit</a>
<a href="/bots/knowledge/{{ $bot->id ?? 'new' }}" class="px-4 py-2 bg-indigo-600 text-white rounded-lg text-sm font-medium hover:bg-indigo-700">Knowledge Base</a>
</div>
                </div>
            </div>
        </div>
...
@empty
    <div class="col-span-full border-2 border-dashed border-slate-200 rounded-xl p-12 text-center my-4">
        ...
        <p class="text-slate-500 text-xs max-w-xs mx-auto mt-1">Your backend agent array returned empty setup fields.</p>
    </div>
@endforelse
    <div id="embed-modal" class="hidden fixed inset-0 z-[9999] flex items-center justify-center p-4 bg-slate-900/40 backdrop-blur-sm">
    <div class="bg-white rounded-2xl shadow-2xl w-full max-w-lg p-6 animate-in fade-in zoom-in duration-200">
        ...
        <p class="text-sm text-slate-500 mb-4">Copy this snippet and paste it into your website's &lt;body&gt; tag.</p>
        
        <textarea id="modal-snippet" class="w-full h-32 text-xs p-4 bg-slate-900 text-indigo-300 rounded-xl mb-4 font-mono shadow-inner" readonly></textarea>
        
        <button onclick="copyModalSnippet()" class="w-full py-2.5 bg-indigo-600 text-white rounded-xl font-medium hover:bg-indigo-700 transition-colors flex items-center justify-center gap-2 shadow-lg">
            <span class="material-symbols-outlined text-sm">content_copy</span> Copy Code
        </button>
    </div>
</div>

      <a href="/bot-config" class="border-2 border-dashed border-slate-200 rounded-xl flex flex-col items-center justify-center p-lg min-h-[220px] hover:bg-slate-50 transition-colors">
                        ...
                        <div class="mt-xl bg-white border border-slate-200 rounded-xl overflow-hidden shadow-sm">
                                ...
                                <div class="overflow-x-auto w-full">
                                        <table class="w-full text-left min-w-[600px]">
                                                ...
                                                <tr>
                                                        <th
                                                                class="px-lg py-3 text-[12px] font-bold text-slate-500 uppercase tracking-wider">
                                                                Event</th>
                                                        <th
                                                                class="px-lg py-3 text-[12px] font-bold text-slate-500 uppercase tracking-wider">
                                                                Bot Source</th>
